...
class AddToRagAction : AnAction() {
    override fun actionPerformed(e: AnActionEvent) {
        ...
        service.addToRag(project, virtualFile)
    }
    ...
    override fun update(e: AnActionEvent) {
        ...
        // Only show and enable if we have a non-directory file that is not already in the RAG folder
        if (virtualFile != null && !virtualFile.isDirectory) {
            val service = RagSnippetService.getInstance()
            val fileExistsInRag = service.fileExistsInRag(virtualFile.name)
            
            // Only show if the file doesn't exist in the RAG folder
            e.presentation.isVisible = !fileExistsInRag
            e.presentation.isEnabled = !fileExistsInRag
        }
        
        println("AddToRagAction.update - Place: ${e.place}, File: ${virtualFile?.name}, Visible: ${e.presentation.isVisible}")
    }
} 